...
const fileContents = require('./mock-data');
...
function generateDates(year) {
  ...
  const date = moment([year + 1723, 0, 1]);
  const offset = moment.duration(timeDelta(year), 'days');
...
function getMarriage(person1, person2, dob1, dob2) {
  ...
  let weddingDate;
  dob1.valueOf() < dob2.valueOf()
    ? (weddingDate = addYears(dob1, 10))
    : (weddingDate = addYears(dob2, 10));
...
for (let person1 of fileContents) {
  const spouses1 = person1['spouses'].split(',');
  const parents1 = person1['parents'].split(',');
  for (let person2 of fileContents) {
    ...
    if (
      spouses1.includes(`${person2['first_name']} ${person2['last_name']}`) &&
      id1 != null &&
      id2 != null
    ) {
    ...
    if (
      parents1.includes(`${person2['first_name']} ${person2['last_name']}`) &&
      id1 != null &&
      id2 != null
    ) {
